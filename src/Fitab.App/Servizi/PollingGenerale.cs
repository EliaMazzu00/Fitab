...
/// abbinamenti del turno in corso e la forcella di posizione che ne deriva.
/// </para>
/// </summary>
public sealed class PollingGenerale(IFitabApi api) : PollingPrudente
{
    ...
    protected override async Task Scarica(CancellationToken ct)
    {
        ...
        try
        {
            var turni = await api.GetLiveTurniAsync(_torneo.Codice, _tessera, ct);
            ...
            if (sequenza.Count == 0)
            {
                ...
                Riuscito();
                await Segnala(new EsitoGenerale(null, DateTimeOffset.Now));
                return;
            }
            ...
            if (righe.Count == 0)
            {
                await Segnala(new EsitoGenerale(_ultima, DateTimeOffset.Now));
                return;
            }
            ...
            await Segnala(new EsitoGenerale(_ultima, DateTimeOffset.Now, turnoNuovo));
        }
